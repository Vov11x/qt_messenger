...
void ChatServer::incomingConnection(qintptr socketDescriptor)
{
    ...
    emit logMessage(QStringLiteral("New client has joined."));
}
...
{
    Q_ASSERT(sender);
    emit logMessage(QLatin1String("Got JSON ") + QString::fromUtf8(QJsonDocument(doc).toJson()));

    QString user_id = doc.value("user-id").toString();
    ...
    if (!user_id.isEmpty()) {
        ...
        if (database.open()) {
            QSqlQuery query;
            QString datetime = doc.value("datetime").toString();
            QString message_content = doc.value("text").toString();
            query.exec("INSERT INTO `messenger_messages` (`user_id`, `datetime`, `message_content`) \
                        VALUES ('"+user_id+"','"+datetime+"','"+message_content+"');");
            database.close();
        }
    }
...
{
    Q_UNUSED(sender)
    emit logMessage(QLatin1String("Error occured by  ") + sender->userName());
}
